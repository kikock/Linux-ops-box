...
>   - **关于修改端口**：若宿主机标准 123 端口已被占用或出于安全隔离需变更端口，可映射为自定义端口（如 `-p 1123:123/udp`）。注意容器内部始终固定监听 123 端口，只需修改冒号左侧的宿主机端口。客户端在连接时指定对应端口即可（Chrony 语法：`server <A_IP> port 1123 iburst`）。
...
> 🚀 **工具箱 B 客户端一键全自动配置与即时同步自检（强烈推荐）**：
> 本工具箱已将 B 客户端的**离线安装、配置写入、服务自启、即时强制步进与状态自检报告**全流程深度集成自动化：
> 1. 在 B 机器上执行：
>    ```bash
>    bash system/system_init.sh
>    ```
